...
(
    SELECT
        C.continent,
        date,
        SUM(C.total_deaths) AS total_deaths_continent,
        SUM(total_cases) AS total_cases_continent,
        SUM(CAST(C.total_deaths AS REAL)) / SUM(total_cases) AS death_percent,
        SUM(total_cases) / CAST(MAX(P.Population) AS REAL) AS HightEfecttion
    FROM PortfolioProject..CovidDeaths AS C
    LEFT JOIN DeathByContinent AS P
    ON C.continent = P.Continent
    WHERE C.continent IS NOT NULL
	GROUP BY C.continent, date
